...
def livro_criar(request):
    if request.method == 'POST':
        ...
        form = LivroForm()
    return render(request,
                  'livraria/livro_form.html',
                  {'form': form, 'titulo_pagina': 'Adicionar Livro'})

def livro_detalhe(request, pk):
    livro = get_object_or_404(Livro, pk=pk)
    return render(request,
                  'livraria/livro_detail.html',
                  {'livro': livro})
...
def livro_editar(request, pk):
    ...
    return render(request,
                  'livraria/livro_form.html',
                  {'form': form, 'livro': livro, 'titulo_pagina': 'Editar Livro'})
...
def livro_excluir(request, pk):
    ...
    return render(request,
                  'livraria/livro_confirm_delete.html',
                  {'livro': livro})
